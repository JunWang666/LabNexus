...
namespace service::MutiWindow {
    ...
    void WindowManager::addWindow(const QString& windowId, QWidget* widget, const QString& title) {
        ...
        connect(widget, static_cast<void (QObject::*)(QObject*)>(&QObject::destroyed), this, &WindowManager::onWindowDestroyed);
    ...
    void WindowManager::removeWindow(const QString& windowId) {
        ...
        if (info.widget) {
            // 断开连接
            disconnect(info.widget, static_cast<void (QObject::*)(QObject*)>(&QObject::destroyed), this, &WindowManager::onWindowDestroyed);
            // 不删除窗口，只是移除管理
            info.widget = nullptr;
        }
    ...
    void WindowManager::closeAllWindows() {
        for (auto it = m_windows.begin(); it != m_windows.end(); ++it) {
            if (it.value().widget) {
                // 断开连接
                disconnect(it.value().widget, static_cast<void (QObject::*)(QObject*)>(&QObject::destroyed), this, &WindowManager::onWindowDestroyed);
                // 关闭并删除窗口
